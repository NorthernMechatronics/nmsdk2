    ...
    switch( Nvm.MacGroup2.DeviceClass )
    {
        case CLASS_A:
        {
            ...
            if( deviceClass == CLASS_C )
            {
                ...
                Nvm.MacGroup2.MacParams.RxCChannel.Frequency = Nvm.MacGroup2.MacParams.Rx2Channel.Frequency;
                Nvm.MacGroup2.MacParams.RxCChannel.Datarate = Nvm.MacGroup2.MacParams.Rx2Channel.Datarate;

                for( int8_t i = 0; i < LORAMAC_MAX_MC_CTX; i++ )
                {
                    if( ( Nvm.MacGroup2.MulticastChannelList[i].ChannelParams.IsEnabled == true ) &&
                        ( Nvm.MacGroup2.MulticastChannelList[i].ChannelParams.RxParams.Class == CLASS_C ) && LmhpRemoteMcastSessionStateStarted())
                    {
...
LoRaMacStatus_t LoRaMacStop( void )
{
    if( LoRaMacIsBusy( ) == false )
    {
        // if( Nvm.MacGroup2.DeviceClass == CLASS_C )
        // {
        //    Radio.Sleep( );      //NMILL: called in NvmDataMgmtStore and interferes with switching to class C Rx. New from 4.6.0->4.7.0
        // }
        MacCtx.MacState = LORAMAC_STOPPED;
        return LORAMAC_STATUS_OK;
    }
