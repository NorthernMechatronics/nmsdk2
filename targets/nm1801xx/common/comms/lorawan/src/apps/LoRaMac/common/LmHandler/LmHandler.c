                                      ...
                                      LmHandlerParams_t *handlerParams )
{
    ...
    else
    {
        ...
        if (CommissioningParams.IsOtaaActivation == false)
        {
            // Tell the MAC layer which network server version are we connecting too.
            mibReq.Type = MIB_ABP_LORAWAN_VERSION;
            mibReq.Param.AbpLrWanVersion.Value = LmAbpLrWanVersion;
            LoRaMacMibSetRequestConfirm( &mibReq );
            ...
            if (CommissioningParams.DevAddr == 0)
            {
...
LmHandlerErrorStatus_t LmHandlerRequestClass( DeviceClass_t newClass )
{
    ...
    // if( currentClass != newClass )                           //Allow loramac to handle
    {
        switch( newClass )
        {
        case CLASS_A:
